...
const DEFAULT_ACTION_OPTIONS = [
  ...
  { value: 'performance', label: 'Special Contribution' },
  { value: 'register_event', label: 'Event Registration' },
]
...
function ClubPointRulesPage({ clubId, isPresident = false, isLeader = false }) {
  ...
  const actionOptions = useMemo(() => {
    const existingActionCodes = new Set(rulesList.map((r) => r.actionValue))
    const rawTypes = actionTypes.length > 0
      ? actionTypes.map((at) => ({ value: at.code, label: at.name, id: at._id, description: at.description }))
      : DEFAULT_ACTION_OPTIONS.filter((opt) => opt.value).map((opt) => ({ ...opt, id: '' }))

    const available = rawTypes.filter((at) => !existingActionCodes.has(at.value))

    if (available.length === 0) {
      return [{ value: '', label: '-- All Action Types Already Configured --', id: '' }]
    }
