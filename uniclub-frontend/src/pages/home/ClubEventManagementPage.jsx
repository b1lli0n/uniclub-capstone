...
function ClubEventManagementPage({ clubId }) {
  ...
  const [events, setEvents] = useState(() =>
    ALL_EVENTS.filter((event) => event.clubId === club.id).map(createManagedEvent)
  )
  const [activeStatus, setActiveStatus] = useState('complete')
  const [detailEvent, setDetailEvent] = useState(null)
  const [editorMode, setEditorMode] = useState(null)
  const [draft, setDraft] = useState(createEmptyDraft(club.id))
  ...
  function openCreateEditor() {
    ...
    setDraft(createEmptyDraft(club.id))
  }
  ...
  function submitEvent(eventSubmit) {
    ...
    const nextEvent = {
      ...draft,
      id: draft.id || `club-event-${club.id}-${Date.now()}`,
      clubId: club.id,
    ...
    <main className="club-event-management-page">
      <section className="club-event-management-hero">
        <div>
          ...
          <h1>Manage Events</h1>
          <p>Create, update, review, and cancel events organized by {club.name}.</p>
        </div>
      ...
      <section className="club-event-management-list" aria-label={`${club.name} managed events`}>
