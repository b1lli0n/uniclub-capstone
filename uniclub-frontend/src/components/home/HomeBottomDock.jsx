  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        ...
        <path d="M7 6H4v2a3 3 0 0 0 3 3M17 6h3v2a3 3 0 0 0-3 3" />
      </svg>
    ),
  },
  {
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        ...
        <path d="M2 8h20v4H2zM12 8v14M12 8H7a2.5 2.5 0 1 1 2.2-3.7L12 8Zm0 0h5a2.5 2.5 0 1 0-2.2-3.7L12 8Z" />
      </svg>
    ),
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        ...
        <path d="M8 2v4M16 2v4M3 10h18M8 14h3M8 18h6" />
      </svg>
    ),
  },
  {
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        ...
        <path d="m4 7 8 6 8-6M16.5 3v4M14.5 5h4" />
      </svg>
    ),
  },
  {
...
function HomeBottomDock({
  ...
  useEffect(() => {
    ...
    if (!el) return
    const active = el.querySelector('.home-bottom-dock__item.is-active')
    if (active) {
  ...
  return (
    <nav className="home-bottom-dock" aria-label="Club navigation" ref={dockRef}>
      {visibleDockItems.map((item) => (
        <button
          key={item.id}
          type="button"
          className={`home-bottom-dock__item${pageId === item.id ? ' is-active' : ''}`}
          onClick={() => onNavigate?.(item.id)}
          aria-current={pageId === item.id ? 'page' : undefined}
          aria-label={item.label}
        >
          <span className="home-bottom-dock__icon">{item.icon}</span>
          <span className="home-bottom-dock__label">{item.label}</span>
        </button>
      ))}
    </nav>
  )
