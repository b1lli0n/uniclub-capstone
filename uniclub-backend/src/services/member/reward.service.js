...
const getMemberRewards = async ({ clubId, userId, search }) => {
  ...
  return {
    available_points: membership.reward_points,
    rewards,
  };
};
...
const getMemberRewardDetail = async ({ clubId, rewardId, userId }) => {
  ...
  return {
    available_points: membership.reward_points,
    can_redeem: reward.quantity > 0 && membership.reward_points >= reward.points_required,
    reward,
  };
};
...
const redeemReward = async ({ clubId, rewardId, userId }) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      if (membership.reward_points < reward.points_required) {
        throw getStatusError("Insufficient reward points", 400);
      }

      membership.reward_points -= reward.points_required;
      reward.quantity -= 1;
