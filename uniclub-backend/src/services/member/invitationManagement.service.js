  ...
  if (!clubId) {
    ...
    creationInvitations = creationRequests.map((req) => {
      ...
      return {
        ...
        role: "member",
        message: `You received an invitation to join as a founding member of ${req.club_name} from ${req.requested_by?.full_name || "President"}`,
  ...
  if (!invitation && mongoose.isValidObjectId(invitationId)) {
    ...
    if (creationReq) {
      ...
      return {
        ...
        role: "member",
        message: `You received an invitation to join as a founding member of ${creationReq.club_name} from ${creationReq.requested_by?.full_name || "President"}`,
        status: memberEntry.status || "pending",
