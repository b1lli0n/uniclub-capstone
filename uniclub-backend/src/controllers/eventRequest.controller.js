...
const createEventRequest = async (req, res, next) => {
  try {
    ...
    const member = await ClubMember.findOne({ club_id: clubId, user_id: req.user.id });
    if (!member) {
      return next(getStatusError("You must be a member of this club to submit an event creation request", 403));
    }
...
const getMyEventRequests = async (req, res, next) => {
  try {
    const userMemberships = await ClubMember.find({ user_id: req.user.id }).select("_id");
...
const reviewEventRequest = async (req, res, next) => {
  try {
    ...
    const note = (review_note || reviewNote || "").trim();
    ...
    if (!status || !["approved", "rejected"].includes(status)) {
      return next(getStatusError("Invalid status. Allowed values: approved, rejected", 400));
    }
    ...
    return res.status(200).json({
      success: true,
      message: `Event creation request ${status}`,
      data: status === "approved" ? { request, event: savedEvent } : request,
    });
  } catch (error) {
    next(error);
  }
